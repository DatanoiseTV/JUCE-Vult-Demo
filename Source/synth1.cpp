...
float Synth1_lfo(Synth1__ctx_type_6 &_ctx, float f, int gate){
   ...
   return (-0.5f + sinf((6.28318530718f * _ctx.phase)));
}
...
float Synth1_process(Synth1__ctx_type_7 &_ctx, float input){
   float lfo_val;
   lfo_val = (_ctx.lfo_amt * Synth1_lfo(_ctx._inst1c8,_ctx.lfo_rate,_ctx.gate));
   float phase1;
   ...
   float phase2;
   phase2 = Synth1_phasor(_ctx._inst4a2,(_ctx.pitch + (32.f * Synth1_smooth(_ctx._inst3c9,(_ctx.detune + lfo_val)))),reset);
   float sine;
...
void Synth1_controlChange(Synth1__ctx_type_7 &_ctx, int control, float value, int channel){
   ...
   if(control == 33){
      _ctx.lfo_amt = (2.f * (-0.5f + (0.00787401574803f * value)));
   }
}
